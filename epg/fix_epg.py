...
def main():
    ...
    for prog in root.findall("programme"):
        if "start" in prog.attrib:
            prog.attrib["start"] = subtract_16_hours(prog.attrib["start"])
        if "stop" in prog.attrib:
            prog.attrib["stop"] = subtract_16_hours(prog.attrib["stop"])
